...
def main() -> None:
    ...
    try:
        ...
        for term_size in range(TERMINAL_MIN, TERMINAL_MAX, 5):
            ...
            print('Flag Combinations and Feature Tests',
                  f'(jobs={args.jobs})',
            )
            print('=' * 60)
...
def run_cases_parallel(cases: list[list[str]],
                       term_size: int,
                       jobs: int,
                       debug: bool) -> TestFailure | None:
    own_bin = './ft_ls'
    if debug:
        own_bin = f'{own_bin}_d'
    ...
    with ctx.Pool(processes=jobs) as pool:
        batches = iter_case_batches(cases=cases,
                                    term_size=term_size,
                                    ft_bin=own_bin,
                                    batch_size=BATCH_SIZE)

        for failure in pool.imap_unordered(run_case_batch,
                                           batches,
                                           chunksize=1):
            if failure is not None:
                pool.terminate()
...
def iter_case_batches(cases: list[list[str]],
                      term_size: int,
                      ft_bin: str,
                      batch_size: int) -> Generator[TestBatch, None, None]:
    iterator = iter(cases)
    while True:
...
def run_case_batch(batch: TestBatch) -> TestFailure | None:
    for args in batch.cases:
        failure = compare_case(term_size=batch.term_size,
                               ft_bin=batch.ft_bin,
                               args=args)
...
def compare_case(term_size: int,
                 ft_bin: str,
                 args: list[str]) -> TestFailure | None:
    ft_cmd = [ft_bin, *args]
    ls_cmd = ['ls', *args]
    ...
    if diff is not None:
        return TestFailure(term_size=term_size,
                           cmd=ft_cmd,
                           kind='stdout',
                           diff=diff)
...
def diff_output(ft_out: str, ls_out: str) -> str | None:
    ...
    diff = ''.join(
        difflib.unified_diff(ft_out_cpy.splitlines(keepends=True),
...
def run_with_pty(cmd: list[str], cols: int = 80) -> PtyResult:
    ...
    return PtyResult(stdout=''.join(chunks['stdout']).replace('\r', ''),
